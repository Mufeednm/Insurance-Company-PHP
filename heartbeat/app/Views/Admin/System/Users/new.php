...
$(document).ready(function() {
    function toggleCompliance() {
        if ($("#userRole").val() === "SALARY") {
            $("#compliance").closest(".col-xxl-3").show();
        } else {
            $("#compliance").closest(".col-xxl-3").hide();
        }
    }
...
<script>
    function toggleStorePermissions() {
        const role = document.getElementById('userRole').value;
        const storePermissions = document.getElementById('storePermissions');
        
        if (role === 'USER' || role === 'SALARY') {
            storePermissions.style.display = 'block';
        } else {
            storePermissions.style.display = 'none';
        }
    }

    // Initial call on page load
    document.addEventListener("DOMContentLoaded", function() {
        toggleStorePermissions();
        document.getElementById('userRole').addEventListener('change', toggleStorePermissions);
    });
</script>
<?= $this->endSection() ?>